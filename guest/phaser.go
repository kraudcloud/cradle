...
func phaser() {
	...
	if CONFIG.Phaser == nil {
		log.Println("cradle: not connecting to phaser")
		return
	}

	urls := []string{}
	for _, url := range CONFIG.Phaser.Url {
		urls = append(urls, fmt.Sprintf("%s/apis/kr.vmm/v1/pod/%s/cradle.yeet.json", url, CONFIG.ID))
	}

	yc, err := yeet.New().
		WithHeader("Authorization", fmt.Sprintf("Bearer %s", CONFIG.Phaser.Token)).
		WithLogger(log).
		Connect(urls...)
	...
	if CONFIG.VDocker != nil {
		fip6 = CONFIG.VDocker.Listen
	}
	if fip6 == "" {
		for _, iff := range CONFIG.Network.Interfaces {
			if iff.HostMode == "fabric" {
...
func reportContainerState(
	...
	for i := 0; i < 3; i++ {
		for _, url := range urls {
			...
			req.Header.Set("Authorization", fmt.Sprintf("Bearer %s", CONFIG.Phaser.Token))
